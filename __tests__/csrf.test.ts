...
describe("CSRF Protection Unit Tests (Vitest)", () => {
  ...
  it("should validate successfully with matching cookie and header", () => {
  ...
  it("should validate successfully with case-insensitive header", () => {
    const token = "some-secure-random-token";
    const req = new MockNextRequest({
      headers: {
        "X-CSRF-Token": token,
  ...
  it("should fail validation if header is missing", () => {
    const token = "some-secure-random-token";
    const req = new MockNextRequest({
      cookies: {
        [CSRF_COOKIE_NAME]: token,
  ...
  it("should fail validation if cookie is missing", () => {
    const token = "some-secure-random-token";
    const req = new MockNextRequest({
      headers: {
        "X-CSRF-Token": token,
  ...
  it("should fail validation if cookie and header mismatch", () => {
    const req = new MockNextRequest({
      headers: {
