...
import type { Roommate } from "./RoommateTable";

interface RoommateStatusPublicProps {
  roommates: Roommate[];
  currentRoommateAddress: string | null;
}

export default function RoommateStatusPublic({ roommates, currentRoommateAddress }: RoommateStatusPublicProps) {
  // Sorting logic: Current roommate first, then others sorted by status
  const sortedRoommates = [...roommates].sort((a, b) => {
    if (a.address === currentRoommateAddress) return -1;
    if (b.address === currentRoommateAddress) return 1;
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-white/5 rounded-2xl border border-white/5 backdrop-blur-md">
         <div className="flex items-center gap-4">
            ...
            <div className="space-y-0.5">
              <h3 className="text-sm font-black text-white uppercase tracking-[0.1em] leading-none">Participant Status</h3>
              <p className="text-[10px] text-dark-500 font-bold uppercase tracking-widest">{roommates.length} Registered Roommates</p>
            </div>
         </div>
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            ...
            <tbody className="divide-y divide-white/5">
              {sortedRoommates.map((roommate, idx) => {
                const isOwnRow = roommate.address === currentRoommateAddress;
                ...
                return (
                  <tr key={idx} className="group hover:bg-white/[0.02] transition-colors">
                           ...
                           <p className="text-sm font-mono text-dark-200 group-hover:text-white transition-colors">
                              {isOwnRow ? (
                                <>
                                  ...
                                  <span className="ml-2 text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-brand-500/20 text-brand-300 border border-brand-500/30">You</span>
                                </>
                              ) : (
                                `${roommate.address.slice(0, 4)}...${roommate.address.slice(-4)} (Anonymized)`
                              )}
                           </p>
                           {isOwnRow && (
                    ...
                    <td className="p-4 text-sm font-bold text-dark-200">
                      {isOwnRow ? `${roommate.expectedShare} XLM` : "—"}
                    </td>
                    <td className="p-4 text-sm font-bold text-white">
                      {isOwnRow ? `${roommate.paidAmount} XLM` : "—"}
                    </td>
                    <td className="p-4 text-sm font-bold text-dark-400">
                      {isOwnRow ? `${remaining} XLM` : "—"}
                    </td>
